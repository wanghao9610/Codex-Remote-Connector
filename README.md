...
The easiest way is to ask Codex to install the plugin for you. Start a Codex thread and paste one of these prompts.
...
Use this only if you want to install the plugin yourself or need to debug the install path.

Codex discovers local plugins through a local marketplace file, then installs the plugin into its own cache. For the default personal marketplace, keep these paths separate:

- Marketplace config: `~/.agents/plugins/marketplace.json`
- Plugin source checkout: `~/plugins/remote-connector`
- Codex installed cache: `~/.codex/plugins/cache/personal/remote-connector/<version>/`

Do not clone the plugin into `~/.agents/plugins/plugins/remote-connector` or `~/.codex/plugins/cache/...`. The marketplace file lives under `~/.agents`, the source checkout lives under `~/plugins`, and the cache under `~/.codex/plugins/cache` is generated by `codex plugin add`.
...
2. Add the plugin to your personal marketplace.
   ...
   If the file already exists, add only the `remote-connector` object to the existing `plugins` array. Do not replace your existing marketplace entries. Keep the source path as `./plugins/remote-connector`; do not change it to an absolute path.
