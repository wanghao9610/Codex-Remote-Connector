...
Codex Remote Connector 用来把一台 SSH 远程机器准备成 Codex desktop app 可以连接的远程开发环境。它会校验本地 SSH alias，在远程机器安装 Codex CLI，启动反向代理隧道，在远程机器执行 Codex device login，并输出 Codex desktop Connections 里需要填写的连接字段。
...
2. 在远程机器创建 `~/.codex` 目录。
3. 将 `scripts/codex_install.sh` 复制到远程机器的 `~/.codex/codex_install.sh`。
4. 在远程机器执行安装脚本。
5. 从远程机器到本地代理启动反向 SSH 隧道：

   ```bash
   ssh -fN -R 127.0.0.1:17890:127.0.0.1:7890 REMOTE_SSH_MACHINE
   ```

6. 在远程机器执行 `codex login --device-auth`。
7. 输出 Codex desktop 连接远程机器所需的配置字段。
...
最简单的方式是直接让 Codex 帮你安装。在一个 Codex thread 中粘贴下面这段指令：

```text
Install the Codex plugin from https://github.com/wanghao9610/Codex-Remote-Connector. Please clone it to ~/plugins/remote-connector, configure the personal marketplace, run codex plugin add remote-connector@personal, and tell me how to use /Remote-Connector after installation.
```

Codex 会处理 clone、personal marketplace 配置、插件安装命令，以及必要的文件写入审批。安装完成后，开启一个新的 Codex thread，然后调用：
...
5. 在 Codex desktop 中添加 SSH connection。
   ...
   打开 **Settings > Connections**，使用脚本输出的字段添加或启用 SSH host：
