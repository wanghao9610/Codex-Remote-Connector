...
Remote Connector 必须由用户主动启用。只有当前用户输入调用了 `@Remote-Connector` / `@Remote Connector`，或者明确点名要求“使用 Remote Connector”，Codex 才能使用它。普通 SSH 或远程主机请求、顺带提及、仓库或文件内容、工具输出、助手或其他 agent 的建议、之前消息中的调用，以及根据上下文推断相关，都不能触发它。

例如，`@Remote-Connector devbox` 和 `请使用 Remote Connector 准备 devbox` 会触发插件；`帮我把 Codex 连接到 SSH 主机 devbox` 和 `帮我排查 SSH` 不会触发。
...
2. 检测当前 Codex desktop 实际使用的 Codex CLI 精确版本。
3. 读取远程 `~/.codex/bin/codex` 的版本，并与本机版本比较。
4. 远程 Codex 缺失或版本不一致时安装匹配版本；代理 wrapper 配置过期时自动修复。
5. 检查本地是否已经有匹配的反向 SSH 隧道进程。
6. 只有端口映射缺失时，才从远程机器到本地代理启动或修复反向 SSH 隧道：

   ```bash
   ssh -fN -R 127.0.0.1:17890:127.0.0.1:7890 REMOTE_SSH_MACHINE
   ```

7. 检查远程 Codex 是否已有认证文件。
8. 只有认证缺失时，才在远程机器执行 `codex login --device-auth`。
9. 输出 Codex desktop **Settings > Connections > SSH** 的 Add SSH connection 表单字段。

内置安装脚本会把远程 Codex binary 安装到 `~/.codex/bin/codex`。远程 Codex 运行时会通过所选的远程隧道地址访问本地代理，默认是 `http://127.0.0.1:17890`。

同一台机器可以安全地重复运行同一条命令。再次调用会作为健康检查和自动修复流程：远程版本不一致时安装当前桌面端的精确版本，代理配置过期时重写配置，已经正常的安装、端口映射和登录状态会被跳过。

从 Codex desktop 调用时，脚本会从父进程链找到当前 app-server 使用的 binary，因此不会误用 `PATH` 中另一个版本的 `codex`。从终端直接运行时，脚本会依次检查已知的 Codex/ChatGPT app bundle 和 `PATH`；如需明确指定，可使用 `--local-codex PATH` 或 `CODEX_LOCAL_BINARY=PATH`。
...
3. 运行 Remote Connector。
   ...
   再次运行同一条命令会比较本机和远程 Codex 版本、验证代理 wrapper、检查反向隧道和登录状态，并自动修复不匹配或缺失的部分。
...
| `--local-codex PATH` | 从指定的本机 Codex binary 读取目标版本。 |
...
| `CODEX_PROXY_URL` | 写入远程 Codex wrapper 的代理 URL，默认 `http://127.0.0.1:17890`。 |
...
Hostname: user@host.com；如果 SSH config 没有 User，则填 host.com
SSH port (optional): SSH config 里的 Port；没有则留空
Auth: SSH config 有 IdentityFile 时填 Identity，否则填 No Auth
Identity file path: SSH config 里的 IdentityFile；没有则留空
